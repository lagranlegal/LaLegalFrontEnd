...
**Verificado en vivo tras desplegar, con el mismo repro que daba 5/5 antes del fix:** 0/5. Ampliado además con un flujo más realista (crear plantilla con contenido real vía "Empezar desde…", escribir texto, insertar un campo dinámico al lado del texto, guardar, cerrar la pestaña del todo y reabrir la plantilla en una pestaña nueva) — cero errores de consola en toda la corrida. Plantillas de prueba borradas al terminar (una quedó visible un momento tras "Plantilla eliminada" — refresco de la lista, no de la eliminación: confirmado con una recarga limpia que ya no estaba).
