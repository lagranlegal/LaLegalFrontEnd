...
El comentario en `PrintLayout.tsx` promete "tiene que verse IDÉNTICO... cero regresión"; la descripción de este mismo documento (línea de arriba, "Las 3 identidades") ya lo describía con más cuidado como "el más parecido al look de siempre", no idéntico — así que es una discrepancia entre lo que dice el código y lo que hace, no un bug escondido. **Pendiente de decisión de Mateo, no arreglado unilateralmente:** ¿el borde doble se queda como mejora estética intencional del look "de toda la vida" (y se corrige el comentario que promete cero regresión), o `classic` vuelve al borde simple exacto de antes para que la garantía de "una empresa que no toca esto no ve NADA distinto" sea literal? Ninguna opción es difícil de implementar — es una decisión de producto, no técnica.
