...
3. Invitación (flujo de alta único — signups públicos OFF): el enlace (por correo, o entregado a mano con «Generar enlace») lleva a `/auth/callback?token_hash=…&type=invite|recovery` → la persona toca **«Continuar»** y recién ahí se canjea con `verifyOtp` (POST; nunca al cargar, para que ni una vista previa que haga GET ni un escáner de correo que ejecute la página lo quemen) → pantalla de crear contraseña (`supabase.auth.updateUser`) → login con esa contraseña → adentro. El backend pasa al usuario de `invited` a `active` con la primera sesión de contraseña (claim `amr`), no con la del enlace. Enlaces viejos o del correo de Supabase (`#access_token=…`) los procesa `detectSessionInUrl` antes de montar. Detalle: `RUNBOOK_USUARIOS.md` (raíz) y el comentario de `AuthCallbackPage.tsx`.
...
| `CASH_SESSION_NOT_OPEN` (409) | **Modal central "Abrir caja"** (`CashSessionRequiredDialog`) con CTA directo a abrir sesión (si tiene `cashbox.open_close`) o aviso de pedirle al responsable. Nunca un toast seco. **También al ANULAR una venta** (F21-02): anular emite un contra-movimiento de caja, así que el backend exige sesión abierta sin importar el medio de pago — el comprobante mostraba un toast, que nombra lo que falta pero no da dónde hacerlo. |
| `CASH_SESSION_ALREADY_OPEN` / `CASH_SESSION_ALREADY_CLOSED_TODAY` (409) | Ninguno de los dos se arregla reintentando, así que el diálogo de abrir caja **no dice "Intenta de nuevo"**: muestra el texto del backend más dónde está la acción que queda (`openSessionErrorMessage` en `features/cashbox/api.ts` — seguir operando, o "Reabrir caja" desde `/caja`, o pedírselo a un responsable), e invalida `['cashbox']` para que el banner global se corrija solo. **El nombre del segundo código estuvo mal ocho meses** (`ALREADY_CLOSED_TODAY`, que el backend nunca emitió): caía a `UNKNOWN` y esta fila entera era código muerto (F20-01). |
...
| `IDEMPOTENCY_IN_PROGRESS` (409) | Llegó el reintento con la misma `Idempotency-Key` **mientras la original seguía en vuelo** — el doble clic en "Vender". No es una falla: banner con el mensaje del backend ("No la repitas: consulta el resultado en unos segundos"), sin reintentar. |
| `MULTIPLE_REGISTERS_NOT_SUPPORTED` (409) | La empresa quedó con más de una caja registradora activa y multi-caja no existe todavía. Hoy no se llega por la API; banner con el mensaje del backend, que dice qué hacer (dejar una sola activa). |
...
| `AUTH_ADMIN_ERROR` (502) | Identity (paso 8): fallo de Supabase Auth Admin al invitar — sin modal específico, banner genérico con `error.message` (ya viene en español, verificado en pruebas reales). |
| `INVITE_RATE_LIMITED` (429) | Se agotó la cuota de correos. **Único código con mensaje propio del front** (`FRONT_MESSAGES` en `lib/api/errors.ts`, servido por `userMessage`): el del backend nombra a Supabase, y quien administra una compraventa no tiene por qué saber qué es eso (F21-06). El reemplazo nombra las dos salidas: esperar unos minutos, o "Generar enlace", que no manda correo. |
...
- **Cuenta vs. medio de pago (`components/shared/AccountPicker`):** transversal desde que existe el catálogo de cuentas. El **medio** dice CÓMO se cobró (`Efectivo`/`Transferencia`/`Otro`); la **cuenta** dice DÓNDE quedó la plata. Los dos selectores van **juntos** en cada punto de cobro (venta, abono, desembolso, gasto, compra) y nunca uno en lugar del otro. `AccountPicker` filtra por el tipo que el medio implica y preselecciona la cuenta por defecto — la misma que elegiría el backend si no se mandara `account_id`; esa correspondencia vive en `lib/accounts/types.ts` y tiene test, porque si se desincroniza la UI mostraría un destino y la plata caería en otro. Detalle y porqué del modelo: `backend-starter/docs/ARCHITECTURE.md` §12.

  **Sin `accounts.view` el selector no se muestra y la operación sigue.** El backend resuelve la cuenta predeterminada cuando no recibe `account_id`, así que bloquear una venta porque el cajero no puede *leer* el catálogo de cuentas sería castigarlo por un permiso que no necesita para cobrar.
...
- **Contrato de códigos de error (`tests/error-codes-contract.test.ts`):** un código es un contrato entre dos capas y **nadie lo compila**, así que se prueba a mano. Los sobres del test están copiados literal de la línea del backend que los emite (con la cita al lado), no escritos de memoria: uno inventado confirma el bug en vez de encontrarlo. Y se afirma sobre el `code`, nunca sobre el status — un test que mira el status no cubre nada de lo que aquí se rompe. `tests/void-sale-cash-session.test.tsx` cubre la otra mitad: que la UI **reaccione** al código (modal de abrir caja, no toast).
