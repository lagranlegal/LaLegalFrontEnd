...
2. ~~**Agregación de caja por rango de fechas**~~ **✅ Resuelto (27/08/2026)** — `GET /reports/closings-breakdown?from_date&to_date` (punto 13.1). Falta conectar el front (`useRawSessions`/`aggregateFinancialSummary` siguen con el N+1 de siempre) — el endpoint ya existe y está probado, el refactor del consumidor queda para su propia sesión.
3. ~~**`?q=` en `GET /contracts`**~~ **✅ Resuelto (27/08/2026)** (punto 2) — busca por número, `legacy_code` y nombre/documento del cliente. Front (`useContractSearch`) ya conectado al filtro real, sin el tope de 200 registros.
4. ~~**`GET /platform/companies/{id}/audit-log`**~~ **✅ Resuelto (27/08/2026)** (punto 14) — un super-admin ya puede ver la auditoría de SEGURIDAD (cambios de roles, remates, anulaciones) de cualquier empresa, no solo la suya. El lado de suscripciones ya estaba resuelto con `subscription_event` (migración 00018, ver punto 25) — eran cosas distintas y ahora las dos existen.
...
## 2. ✅ Resuelto (27/08/2026) — `GET /contracts` sin `?q=` — buscador de contratos era un parche client-side
...
**✅ `?customer_id=` resuelto (27/08/2026):** `GET /contracts` ya lo acepta (mismo patrón que `?customer_id=` de `GET /sales`, punto 3 abajo) — resuelve el historial de cliente, que era el consumidor real de este parche.

**✅ `?q=` resuelto (27/08/2026, mismo día).** Busca por número de contrato (prefijo sobre el texto), `legacy_code` (prefijo, sin distinguir mayúsculas) y cliente — nombre (full-text español) o documento (prefijo), vía `LEFT JOIN` contra `customer` (mismo criterio que ya usaba `customers.list_customers` para su propio `?q=`). El front (`useContractSearch`, `features/contracts/api.ts`) sigue trayendo hasta 200 y filtrando en el navegador — **ese parche client-side no se tocó todavía**, solo se resolvió el hueco del backend; conectar el front al filtro real es el paso que falta para cerrar el punto del todo.
...
**✅ Punto 1 resuelto del lado del backend (27/08/2026) — `GET /reports/closings-breakdown?from_date&to_date`.** Mismas columnas que `GET /cashbox/sessions/{id}/report` (módulo × concepto × medio × cuenta), sumadas sobre TODAS las sesiones cerradas del rango en una sola consulta (agrupa por `session_date`, no por sesión — la regla "una sesión por día" hace que sean equivalentes) — reemplaza el N+1 real que hacía `useRawSessions` (una request por sesión del rango, hasta ~90 para 3 meses). Mismos dos permisos que `/closings` (`reports.view` + `cashbox.view_history`), mismo criterio de "sin tope server-side, el caller decide" que ya tenía `/closings`. Test de integración compara línea por línea contra `/cashbox/sessions/{id}/report` para una sesión conocida.

**Lo que falta para cerrarlo del todo — CONECTAR el front:** `useRawSessions`/`aggregateFinancialSummary` (`features/reports/api.ts`, `features/reports/aggregate.ts`) siguen haciendo el N+1 de siempre; no se tocaron todavía porque `aggregateFinancialSummary` itera `sessions: {sessionDate, report}[]` y cambiar su forma de entrada (a una lista plana de líneas con `session_date` embebido) es un cambio real sobre lógica financiera con varias reglas no obvias ya documentadas en sus comentarios (qué cuenta como flujo, capital vs. ingreso, etc.) — mejor hacerlo con cuidado en su propia sesión que apurado. La pieza que faltaba (el endpoint) ya existe y está probada; falta el refactor del consumidor.

Lo que SIGUE bloqueado por backend, en orden de prioridad:
1. **`GET /reports/series?months=12`** — ya estaba en el backlog aceptado del backend (`docs/pending/API_GUIDE.md` §15) desde antes de este documento. Sin esto, la "tendencia diaria" de `/reportes` solo puede cubrir el mismo tope de 90 días de antes (el nuevo `/closings-breakdown` no tiene ese tope server-side, pero el front sigue sin pedir más de 90 días de una).
2. **Filtros de fecha en `GET /sales`** — mismo punto 3 de la primera revisión de este documento. Los TOTALES de venta del rango sí salen de la agregación de caja (arriba), pero el detalle por factura individual dentro de un rango sigue sin poder listarse. **Actualización (19/08/2026):** también bloquea a "prendas más vendidas"/"categorías más movidas" (pedidas por el cliente en el rediseño v2) — se construyeron igual, pero como **histórico completo** (todas las ventas de siempre, no el rango elegido en el date picker), rotulado explícitamente así en la UI — decisión confirmada con el cliente en vez de fingir un filtro de fecha que el backend no soporta. Con este filtro, podrían acotarse al rango como el resto de la pantalla.
3. **Cartera histórica** (`capital_outstanding` en una fecha pasada, no solo "hoy"). `/reportes` muestra la cartera actual como snapshot rotulado "corte de hoy", separado a propósito del rango elegido — no hay forma de mostrar "cartera al 15/07" sin esto.
...
**✅ Resuelto (27/08/2026) — `GET /platform/companies/{id}/audit-log`.** Exactamente el molde que se había anotado: `require_super_admin` + sesión `get_db` (bypass de RLS) + `company_id` siempre explícito en el `WHERE` de la query (nunca confiado a RLS — defensa en profundidad, aunque el bypass ya lo permitiría). Reusa `audit.repository.list_audit_log` tal cual (esa query ya filtraba por `company_id` a mano, no dependía de RLS para acotar), en vez de duplicar el SQL — mismo criterio que ya usa `platform` para `identity.repository` en otros puntos del módulo. Mismos filtros que el endpoint tenant-scoped (`module`, `entity_type`, `entity_id`, `user_id`, cursor). Test de integración verifica las dos direcciones: el super-admin SÍ ve la auditoría de la empresa consultada, y NO ve la de otra empresa distinta aunque ambas tengan filas — el punto entero del fix. `amount_paid` (historial de pagos de suscripción) sigue pendiente, sin tocar.
