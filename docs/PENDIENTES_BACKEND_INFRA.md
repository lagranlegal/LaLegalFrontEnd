...
2. ~~**Agregación de caja por rango de fechas**~~ **✅ Resuelto de punta a punta (27-28/08/2026)** — `GET /reports/closings-breakdown?from_date&to_date` (punto 13.1) + front conectado el 28/08 (`useClosingsBreakdown`, ver `docs/IMPLEMENTATION.md`). El N+1 de `useRawSessions` ya no existe.
...
**✅ Cerrado del todo — front conectado (28/08/2026).** `useClosingsBreakdown` (`features/reports/api.ts`) reemplaza el N+1. `aggregateFinancialSummary` cambió su forma de entrada a una lista plana de líneas + `sessionDates` aparte (una sesión sin movimientos no deja línea en el desglose, pero igual cuenta) — mismas reglas de negocio, sin tocar (`addLine` intacto). Queda un N+1 más chico y ya acotado: `GET /cashbox/expenses` por sesión, para la categoría de gasto, dimensión que `closings-breakdown` no cubre. Verificado en vivo: cero llamadas al endpoint viejo, filtro de módulo sin refetch, números correctos al filtrar. Detalle en `docs/IMPLEMENTATION.md`.
